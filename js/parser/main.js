...
(function init() {
  ...
  const copyActions = {
    ...
    copyUUIDSQL() {
      if (state.uuids.length) {
        const text = "in (" + state.uuids.map(u => `'${u}'`).join(',') + ")";
        copyToClipboard(text).then(() => dom.uuidSqlOutput && showCopied(dom.uuidSqlOutput));
      }
    },
    copyUUIDCsv() {
      if (state.uuids.length) copyToClipboard(state.uuids.join(', ')).then(() => dom.uuidCsvOutput && showCopied(dom.uuidCsvOutput));
    },
  ...
  window.copyUUIDSQL = copyActions.copyUUIDSQL;
  window.copyUUIDCsv = copyActions.copyUUIDCsv;
