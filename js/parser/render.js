...
export function render(dom, state) {
  ...
  if (hasNumbers || hasUUIDs || hasGRZ) {
    ...
    if (dom.cardUuidSql) dom.cardUuidSql.style.display = hasUUIDs ? 'block' : 'none';
    if (dom.cardUuidCsv) dom.cardUuidCsv.style.display = hasUUIDs ? 'block' : 'none';

    if (hasUUIDs) {
      dom.uuidOutput && (dom.uuidOutput.textContent = state.uuids[0]);
      dom.uuidSqlOutput && (dom.uuidSqlOutput.textContent = `in ('${state.uuids.join("','")}')`);
      dom.uuidCsvOutput && (dom.uuidCsvOutput.textContent = state.uuids.join(', '));
      dom.uuidMainCount && (dom.uuidMainCount.textContent = `1/${state.uuids.length}`);
      dom.uuidSqlCount && (dom.uuidSqlCount.textContent = `${state.uuids.length}`);
      dom.uuidCsvCount && (dom.uuidCsvCount.textContent = `${state.uuids.length}`);
